...
const int Ma = 2e5 + 1, inf = 0x3f3f3f3f;
const ll mod = 1000000007, k = 28;//212370440130137957, k = 233;
...
const int Maxx = 1000007;
int ans = 0;
bool hs[Maxx];

typedef int sta[26];
const int vsiz = sizeof(sta);

ll sum[Ma];

inline void Hash(const int& pos, const int& len, const ll& tp) {
	//debug(sum[pos-1], tp);
	//debug(sum[pos-1]*tp);
	ll no = (sum[pos + len - 1] - (pos == 0 ? 0 : sum[pos - 1] * tp % mod) + mod) % mod;
	no %= Maxx;
	if (!hs[no]) ++ans, hs[no] = true;
	//int now = 0;
	//for (int i = 0; i < len; i++)
	//	now = (now * k % mod + t[pos + i] - 'a' + 1) % mod;
	//cerr << now << endl;
	//cerr << no << endl;
}
...
signed main() {
	...
	sta ss; memset(ss, 0, vsiz);
	for (int i = 0; i < ls; i++)
		++ss[s[i] - 'a'];

	sta tt; memset(tt, 0, vsiz);
	ll now = 0;
	for (int i = 0; i < ls - 1; i++)
		++tt[t[i] - 'a'], now = (now * k % mod + t[i] - 'a' + 1) % mod, sum[i] = now;
	for (int i = ls - 1; i < lt; i++) {
		now = (now * k % mod + t[i] - 'a' + 1) % mod;
		sum[i] = now;
		++tt[t[i] - 'a'];
		if (i != ls - 1) --tt[t[i - ls] - 'a'];

		if (memcmp(ss, tt, vsiz) == 0)
			Hash(i - ls + 1, ls, tp);
	}
