/*************************************************************************
    ...
    > Created Time: 2020-07-19 14:15:24
 ************************************************************************/
...
const double P = acos(-1.0), eps = 1e-6;
struct point { db x ,y;};
...
const db res[] = {1.0, 1.7, 2.0, 2.3, 2.7, 3.0, 3.3, 3.7, 4.0, 4.3};
const int lim[] = {60, 62, 65, 67, 70, 75, 80, 85, 90, 95};
db dp[Ma];
...
signed main() {
	...
	while (T--) {
		int n; cin >> n;
		if (n <= 59) cout << "0.0" << endl;
		else {
			db ans = 0; fill(dp, dp + n + 1, 0);
			for (int i = 0; i < 10; i++)
				for (int j = lim[i]; j <= n; j++)
					dp[j] = max(dp[j], dp[j - lim[i]] + res[i]),
					ans = max(ans, dp[j]);
