/*************************************************************************
    ...
    > Created Time: Mon Jul 13 14:46:07 2020
 ************************************************************************/
...
namespace Geo{
	...
	inline bool onLine(Point c, Point a, Point b) {
		return sign(cross(b - a, c - a)) == 0;
	}
...
const int Ma = 2e3 + 100, inf = 0x3f3f3f3f, mod = 1e9 + 7;
...
int main() {
	...
	for (int i = 0; i < n; ++i) {
		...
		for (int j = 0; j < n; j++) if (j != i) {
			pt[k].o = pp[j] - pp[i];
			pt[k].gen();
			if (sign(pp[i].x) == 0) {++er; continue;}
			else if (cmp(pt[k].res, pp[i].y / pp[i].x) == 0) continue;
			++k;
		}
		...
		int cnt = 2;
		for (int j = 1; j < k; j++) {
			if (cmp(pt[j - 1].res, pt[j].res) == 0) ++cnt;
