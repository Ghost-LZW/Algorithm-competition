...
const int Ma = 2e3 + 10, inf = 0x3f3f3f3f, mod = 1e9 + 7;
...
int main() {
	...
	for (int i = 0; i < n; i++)
		for (int j = i + 1; j < n; j++) {
			if (same(res[i], res[j])) continue;
			tp[cnt++] = Geo::getcircle(point(0, 0), res[i], res[j]).o;
		}
	sort(tp, tp + cnt);
	int ans = 0;
	for (int i = 0; i < cnt; ++i) {
		int t = 1;
		while (i + 1 < cnt and tp[i] == tp[i + 1]) ++i, ++t;
		ans = max(ans, t);
	}

	for (int i = 1; i <= n; i++) if (i * (i - 1) / 2 == ans) {
		ans = i; break;
	}
