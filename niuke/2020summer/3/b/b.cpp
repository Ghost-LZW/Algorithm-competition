/*************************************************************************
    ...
    > Created Time: 2020-07-18 12:07:08
 ************************************************************************/
...
signed main() {
	...
	while (q--) {
		char a; int b;
		cin >> a >> b;
		if (a == 'A') cout << val[b - 1] << endl;
		else {
			if (b > 0) val.insert(val.size(), val.substr(0, b)), val.erase(0, b);
			else val.insert(0, val.substr(val.size() + b, -b)), val.erase(val.size() + b, -b);
		}
	}
