...
bool subCmp(pair<int, int> A, pair<int, int> B) {
	int k1 = A.first, k2 = A.second, k3 = B.first, k4 = B.second;
	if (k1 == k3) return k2 < k4;
	int la = k2 - k1 + 1, lb = k4 - k3 + 1;
	int a = sa.compare(k1 - 1, k3 - 1, min(la, lb));
	/*cerr << k1 << ' ' << k2 << ' ' << la << ' '
		 << k3 << ' ' << k4 << ' ' << lb << endl;
	cerr << a << endl;*/
